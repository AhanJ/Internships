...
app.post("/auth/register/admin", async (req, res) => {
  try {
    ...
    if (!val) {
      ...
      var token = jwt.sign({ UserID: info._id, Role: info.role }, key, {
        expiresIn: 600,
      });

      res
        .status(201)
        .send("Admin Registration Successful.\nToken Generated: " + token);
...
app.post("/auth/register/user", async (req, res) => {
  try {
    ...
    if (!val) {
      ...
      res
        .status(201)
        .send("User Registration Successful.\nToken Generated: " + token);
      ...
      if (info.role == "admin") {
        bcrypt.compare(req.body.password, info.password).then((out) => {
          if (out == true) {
            ...
            res.send(
              "Admin Login Successful. Access Granted.\nToken Generated: " +
                token
            );
...
app.get("/auth/login/user", async (req, res) => {
  try {
    ...
    if (val) {
      const info = await details.findOne({
        email: req.body.email,
        role: "user",
      });
      ...
      if (info.role == "user") {
        bcrypt.compare(req.body.password, info.password).then((out) => {
          if (out == true) {
            ...
            res.send(
              "User Login Successful. Access Granted.\nToken Generated: " +
                token
            );
