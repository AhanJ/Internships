...
app.post('/API3', (req,res) => { // API3 calculates sum of a randomly generated number along with received number from API2
...
app.get('/API2', async (req,res) => { // API2 generates a random number and sends the result to API3
...
app.get('/API1', async (req,res) => { // API1 calls API2
    ...
    res.send('Random Number (API2): ' + apiResponse.data.a + '\nRandom Number (API3) ' + apiResponse.data.b + '\nSum: ' + apiResponse.data.sum + '\nAhan')
})